...
fn main() {
    ...
    if let Some(ref vm) = quota_report.volume_measurement {
        ...
        println!("   Downlink Volume: {} bytes", vm.downlink_volume.unwrap_or(0));
    }
    println!();
    ...
    if let Some(ref dm) = time_report.duration_measurement {
        println!("   Duration: {} seconds ({} hours)", dm.duration_seconds, dm.duration_seconds / 3600);
    }
    println!();

    // Example 3: Start of traffic report with packet timing
    println!("3. Start of Traffic Report with Packet Timing:");
    let start_report = UsageReportBuilder::start_of_traffic_report(
        UrrId::new(3),
    ...
    println!("   Trigger: {:?}", comprehensive_report.usage_report_trigger);

    if let Some(ref vm) = comprehensive_report.volume_measurement {
        println!("   Volume Statistics:");
        println!("     Total: {} GB", vm.total_volume.unwrap_or(0) / 1_000_000_000);
        println!("     Uplink: {} GB", vm.uplink_volume.unwrap_or(0) / 1_000_000_000);
        println!("     Downlink: {} GB", vm.downlink_volume.unwrap_or(0) / 1_000_000_000);
        ...
        println!("     Downlink: {} packets", vm.downlink_packets.unwrap_or(0));
    }

    if let Some(ref dm) = comprehensive_report.duration_measurement {
        println!("   Session Duration: {} seconds ({:.1} hours)",
                dm.duration_seconds, dm.duration_seconds as f64 / 3600.0);
    }
    ...
    match rs_pfcp::ie::usage_report::UsageReport::unmarshal(&original_data) {
        Ok(unmarshaled) => {
            ...
            println!("   ✓ Data integrity verified: {}",
                    if unmarshaled == comprehensive_report { "PASS" } else { "FAIL" });
        }
    ...
    let volume_only = UsageReportBuilder::new(UrrId::new(10))
        ...
        .build()
        .unwrap();
    println!("   Volume-only report: {} bytes total",
            volume_only.volume_measurement.as_ref().unwrap().total_volume.unwrap());
    ...
    let packet_only = UsageReportBuilder::new(UrrId::new(11))
        ...
        .build()
        .unwrap();
    println!("   Packet-only report: {} packets total",
            packet_only.volume_measurement.as_ref().unwrap().total_packets.unwrap());
    ...
    let timing_only = UsageReportBuilder::new(UrrId::new(12))
        ...
        .build()
        .unwrap();
    println!("   Timing-only report: first=0x{:08X}, last=0x{:08X}",
