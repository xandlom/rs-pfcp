...
fn main() {
    ...
    let response = SessionDeletionResponse::new(1, 4, cause_ie, None, vec![]);
...
fn test_message_length<T: Message>(message: &T, msg_name: &str) {
    ...
    if marshaled.len() >= 20 {
        ...
        let seid = if s_flag {
            u64::from_be_bytes([
                marshaled[4], marshaled[5], marshaled[6], marshaled[7],
                marshaled[8], marshaled[9], marshaled[10], marshaled[11]
            ])
        } else { 0 };
        let seq_offset = if s_flag { 12 } else { 4 };
        let sequence = u32::from_be_bytes([0, marshaled[seq_offset], marshaled[seq_offset + 1], marshaled[seq_offset + 2]]);
        ...
        println!("  Raw bytes: {:02x?}", &marshaled[..marshaled.len().min(24)]);
