...
use rs_pfcp::ie::{
    ...
    // Core IEs
    NodeId, fseid::Fseid, cause::{Cause, CauseValue}, CreatePdr, CreateFar, Pdi, 
    SourceInterface, DestinationInterface, ApplyAction, Precedence, PdrId, FarId,
};
use rs_pfcp::message::{
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    println!("🌟 Advanced Session Management - Complete 5G Lifecycle");
    println!("======================================================");
    ...
    println!("\n🎉 Complete 5G session lifecycle demonstrated!");
    println!("📊 Features used: All Phase 1-3 implementations");
...
fn association_setup_with_ha() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let association_request = AssociationSetupRequestBuilder::new(1)
        .node_id(NodeId::new_ipv4(Ipv4Addr::new(10, 1, 0, 100)).into()) // SMF IP
        .recovery_time_stamp(SystemTime::now())
        .build();

    println!("📤 SMF → UPF: Association Setup Request");
...
fn session_establishment_multi_access() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let fseid = Fseid::new(session_id, Ipv4Addr::new(10, 1, 0, 100));
    ...
    println!("📤 SMF → UPF: Session Establishment Request");
    ...
    let session_response = SessionEstablishmentResponseBuilder::accepted(session_id, 10)
        .node_id(NodeId::new_ipv4(Ipv4Addr::new(10, 1, 0, 200)).into())
        .fseid(0x987654321FEDCBA0, Ipv4Addr::new(10, 1, 0, 200))
        .build();

    println!("📥 UPF → SMF: Session Establishment Response");
    println!("✅ Multi-access session established successfully");
...
fn session_modification_usage_reporting() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let modification_request = SessionModificationRequestBuilder::new(session_id, 20)
        .query_urrs(vec![
    ...
    println!("📤 SMF → UPF: Session Modification Request");
    ...
    println!("✅ On-demand usage reporting requested");
...
fn network_resilience_recovery() -> Result<(), Box<dyn std::error::Error>> {
    println!("\n🛡️ Step 4: Network Resilience and Recovery");
    println!("------------------------------------------");
    ...
    println!("🔄 Session Set Management:");
    ...
    println!("✅ Network resilience mechanisms active");
