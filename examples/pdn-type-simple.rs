...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (pdn_ie, name) in [
        ...
        println!("   • {}: Type={}, Supports IPv4={}, IP-based={}", 
                 name, pdn_ie.payload[0], pdn_type.supports_ipv4(), pdn_type.is_ip_based());
    }
    ...
    if let Some(pdn_ie) = deserialized.find_ie(IeType::PdnType) {
        let pdn_type = PdnType::unmarshal(&pdn_ie.payload)?;
        println!("   📋 Preserved PDN Type: {:?} (supports IPv4: {}, supports IPv6: {})",
                 pdn_type.pdn_type, pdn_type.supports_ipv4(), pdn_type.supports_ipv6());
    }
