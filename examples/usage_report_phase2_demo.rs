...
fn main() {
    ...
    let quota_exhausted = UsageReportBuilder::quota_exhausted_report(
        UrrId::new(1),
    ...
    if let Some(ref vq) = quota_exhausted.volume_quota {
        println!("   Volume Quota - Total: {:?}, UL: {:?}, DL: {:?}",
    ...
    let time_quota_report = UsageReportBuilder::time_threshold_report(
        UrrId::new(2),
    ...
    if let Some(ref tq) = time_quota_report.time_quota {
        println!("   Time Quota: {} seconds ({})", tq.quota_seconds,
    ...
    let complex_report = UsageReportBuilder::periodic_usage_report(
        UrrId::new(3),
    ...
    if let Some(ref vq) = complex_report.volume_quota {
        println!("   Volume Quota - Total: {:?} bytes, Uplink: {:?} bytes",
    ...
    let _max_values_report = UsageReportBuilder::volume_threshold_report(
        UrrId::new(4),
        SequenceNumber::new(45)
    )
    .volume_quota(VolumeQuota::new(0x07, Some(u64::MAX), Some(u64::MAX), Some(u64::MAX)))
    ...
    println!("   Timestamps: Start 0x{:08X}, End 0x{:08X}", u32::MAX, u32::MAX);
    println!();

    // Example 5: Zero Values Test
    println!("5. Zero Values Test:");
    let zero_values_report = UsageReportBuilder::start_of_traffic_report(
        UrrId::new(5),
        SequenceNumber::new(46)
    )
    ...
    let _individual_setters = UsageReportBuilder::periodic_usage_report(UrrId::new(6), SequenceNumber::new(47))
        .volume_quota(VolumeQuota::new(0x07, Some(1000000), Some(600000), Some(400000)))
        .time_quota(TimeQuota::new(1800))
        .build()
        .expect("Failed to build individual setters example");

    // Using convenience methods
    let _convenience_methods = UsageReportBuilder::periodic_usage_report(UrrId::new(7), SequenceNumber::new(48))
